# Learn Workflow — /autonexus:learn

Autonomous codebase documentation engine. Scouts codebase structure, learns patterns and architecture, generates/updates comprehensive documentation — then validates and iteratively improves until docs are accurate. Syncs knowledge to Obsidian Knowledge Center for cross-session persistence.

**Core idea:** Scout → Analyze → Map → Generate → Validate → Fix → Finalize → Log. The Knowledge Center in Obsidian is the natural partner: learn produces the knowledge, pre-push rebuilds sync it, predict consumes it.

## Trigger

- User invokes `/autonexus:learn`
...
**Cancel handling:** If user selects "Cancel" → exit: "Learning cancelled. Run `/autonexus:learn` when ready."

## Architecture

```
/autonexus:learn
  ├── Phase 1: Scout — Parallel codebase reconnaissance (+ Obsidian Knowledge Center cache check)
  ├── Phase 2: Analyze — Structure detection + project type classification
  ├── Phase 3: Map — Dynamic doc discovery + gap analysis
  ├── Phase 4: Generate — Spawn docs-manager with structured prompt (+ write to Obsidian Knowledge Center)
  ...
  ├── Phase 7: Finalize — Size check, inventory, git diff summary (+ update Obsidian Knowledge Center metadata)
...
### Obsidian Knowledge Center as Scout Cache (Phase 1 addition)

Only if `OBSIDIAN_AVAILABLE = true`. Before running the full codebase scout, check if Obsidian Knowledge Center has recent, usable data:

```
TRY:
  arch = obsidian_read_note({ filePath: "Projects/{ProjectName}/Knowledge/Architecture.md", includeStat: true })
  comp = obsidian_read_note({ filePath: "Projects/{ProjectName}/Knowledge/Components.md", includeStat: true })
  deps = obsidian_read_note({ filePath: "Projects/{ProjectName}/Knowledge/Dependencies.md", includeStat: true })
CATCH:
  Log: "Obsidian Knowledge Center not found or unreadable — proceeding with full scout"
  SKIP cache logic, continue with standard scout
```

**Cache evaluation:**
1. Extract `commit_hash` from each Knowledge Center note's frontmatter
2. Compare against current `git rev-parse HEAD`
3. If `commit_hash` matches HEAD → **cache hit**: use Knowledge Center content as scout context baseline. Still run scouts, but they can be lighter (skip deep directory scans for areas already documented).
4. If `commit_hash` is within `git log --oneline HEAD~20` → **partial cache**: use as baseline, run targeted re-scout on changed directories via `git diff --name-only {cached_hash}..HEAD`.
5. If `commit_hash` is older or missing → **cache miss**: full scout as normal.

This is optimization, not replacement. Full scout is always the fallback. Cache context supplements scout reports.

Uses retry-then-queue protocol for the read calls. If Obsidian reads fail, proceed with full scout.
...
Output: `Phase 1: Scouted — [N] files, [M] directories, [K] LOC analyzed [+ Obsidian cache: {hit|partial|miss}]`
...
### Obsidian Knowledge Center Sync (Phase 4 addition)

Only if `OBSIDIAN_AVAILABLE = true`. In addition to writing docs/ locally, ALSO write/update the Obsidian Knowledge Center with the same analysis. This is the natural production path: learn generates the knowledge, Obsidian stores it persistently, pre-push rebuilds keep it current.

After the docs-manager completes, extract knowledge and write to Obsidian:

**Architecture.md** — derived from `docs/system-architecture.md` + `docs/codebase-summary.md`:
```
obsidian_update_note({
  targetType: "filePath",
  targetIdentifier: "Projects/{ProjectName}/Knowledge/Architecture.md",
  content: "{rendered Architecture.md per obsidian-integration.md template}",
...
**Components.md** — derived from `docs/code-standards.md` + `docs/system-architecture.md`:
```
obsidian_update_note({
  targetType: "filePath",
  targetIdentifier: "Projects/{ProjectName}/Knowledge/Components.md",
  content: "{rendered Components.md per obsidian-integration.md template}",
  modificationType: "wholeFile",
  wholeFileMode: "overwrite",
  createIfNeeded: true
})
...
**Dependencies.md** — derived from `docs/codebase-summary.md` (Key Dependencies section) + scout import analysis:
```
obsidian_update_note({
  targetType: "filePath",
  targetIdentifier: "Projects/{ProjectName}/Knowledge/Dependencies.md",
  content: "{rendered Dependencies.md per obsidian-integration.md template}",
  modificationType: "wholeFile",
  wholeFileMode: "overwrite",
  createIfNeeded: true
})
...
Set frontmatter on each Knowledge Center note:
```
...
All Obsidian writes use retry-then-queue protocol. If writes fail, the local docs/ files are still the authoritative output.
...
Output: `Phase 4: Generated — [N] docs created/updated [+ Obsidian Knowledge Center: {synced|queued|skipped}]`
...
## Phase 6: Fix — Validation-Fix Loop (AUTONEXUS CORE)

**Only entered if `validation_score < 100%` AND `--no-fix` is NOT set.**

This is the core autonexus iteration pattern applied to documentation.
...
### Obsidian Knowledge Center Metadata Update (Phase 7 addition)

Only if `OBSIDIAN_AVAILABLE = true` AND Knowledge Center was written in Phase 4. Update the `commit_hash` and `updated_at` on all three Knowledge Center notes to reflect the finalized state:
...
This ensures the Knowledge Center frontmatter reflects the commit hash at finalization time (which may differ from Phase 4 if docs were fixed and committed in Phase 6).

Uses retry-then-queue protocol for all frontmatter updates.

Output: `Phase 7: Finalized — [N] files ready [+ Obsidian Knowledge Center: metadata updated]`
...
```tsv
iteration	mode	docs_generated	docs_updated	validation_score	fix_iterations	learn_score	obsidian_sync	duration_s
1	init	7	0	100	1	95	synced	45
2	update	0	5	85	2	78	queued	62
```
...
Learn score: [S]
Obsidian Knowledge Center: [synced|queued|skipped]
```
...
- Obsidian Knowledge Center sync status
...
| Write Obsidian Knowledge Center without local docs | Obsidian is a mirror, not the source — local docs/ is authoritative | Always write local docs first, then sync to Obsidian |
| Skip Obsidian cache in Phase 1 | Wasting time re-scouting what was just analyzed last session | Check Knowledge Center commit_hash before full scout |
...
## Relationship to Obsidian Knowledge Center

The learn workflow is the natural **producer** of knowledge that flows into Obsidian:

```
/autonexus:learn (init/update)
  → writes docs/ locally (authoritative)
  → syncs Architecture.md, Components.md, Dependencies.md to Obsidian Knowledge Center
  → sets commit_hash + updated_at in frontmatter

Pre-push knowledge rebuild (obsidian-integration.md)
  → re-scans codebase at push time
  → refreshes Knowledge Center if stale
  → ensures pushed code and Obsidian knowledge are in sync

/autonexus:predict
  → reads Knowledge Center as scout cache
  → benefits from learn's thorough analysis without re-scouting
```

This creates a virtuous cycle: learn → Obsidian → predict/pre-push → learn again.
