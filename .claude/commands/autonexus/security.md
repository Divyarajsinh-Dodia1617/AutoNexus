---
name: autonexus:security
description: Autonomous security audit — STRIDE threat model + OWASP Top 10 + red-team with 4 adversarial personas + Obsidian knowledge backbone
...
2. Read the security workflow: `.claude/skills/autonexus/references/security-workflow.md`
3. If scope is missing — use `AskUserQuestion` with batched questions per security-workflow.md
4. Execute the 7-phase security audit (Recon → Assets → Trust Boundaries → STRIDE → Attack Surface → Loop → Report)
