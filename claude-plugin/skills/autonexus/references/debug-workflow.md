# Debug Workflow — /autonexus:debug

Autonomous bug-hunting loop that applies the scientific method iteratively. Doesn't stop at one bug — keeps investigating until the codebase is clean or you interrupt. Findings are persisted to Obsidian for cross-session memory.

**Core idea:** Hypothesize -> Test -> Prove/Disprove -> Log -> Repeat. Every finding needs code evidence. Every failed hypothesis teaches the next one. Every confirmed bug becomes an Obsidian Decision note.

## Trigger

- User invokes `/autonexus:debug`
...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autonexus:debug` is invoked without `--scope` or `--symptom`, you MUST use `AskUserQuestion` to gather full context BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.
...
| 4 | `After` | "When bugs are found, should I also fix them?" | "Find bugs only (report)", "Find and fix (chain to /autonexus:fix)", "Ask me after each finding" |
...
/autonexus:debug
  ...
  ├── Phase 2: Reconnaissance (scan codebase, map error surface, query Obsidian)
  ...
  ├── Phase 6: Log (record finding locally + write to Obsidian)
...
Understand the codebase area where bugs likely live. **This phase includes Obsidian knowledge queries.**
...
**Obsidian Action 1: Query Past Debug Sessions**

Search for past debug sessions involving the same files to avoid repeating failed investigations:

```
FOR each file in the error surface:
...
Parse results:
  ...
  - Feed into hypothesis formation: "Avoid: {description} — previously discarded because {reason}"
  - Look for notes where status: keep — extract what worked for similar issues
```

**Obsidian Action 2: Check Prediction Warnings**

Check if any `/autonexus:predict` run flagged concerns about files in the error surface:
...
Parse results:
  ...
  - Feed into hypothesis formation: "Warning from {persona}: {severity} — {recommendation}"
  - HIGH/CRITICAL warnings should be prioritized as hypotheses
```

Both Obsidian queries use the retry-then-queue protocol. If both fail, proceed with git history + local data only. No warning needed — graceful degradation.
...
**Output:** `Phase 2: Recon — [N] files scanned, [M] potential failure points mapped, [K] Obsidian history matches`
...
| 4 | **Obsidian persona warning** | Predict run flagged this file/pattern |
| 5 | **Environment diff** | Works locally, fails in CI/prod |
| 6 | **Dependency issue** | After upgrade/install |
| 7 | **Race condition** | Intermittent, timing-dependent |
| 8 | **Edge case** | Works for most inputs, fails for specific ones |
...
Obsidian history matches used: 2
```
...
**Per-confirmed-bug Decision note:** Each confirmed bug gets a Decision note in Obsidian with full details:

```markdown
---
tags: [autonexus/decision, autonexus/bug, autonexus/{project}]
date: {YYYY-MM-DD}
status: confirmed
...
## Context
{How the bug was discovered. What symptom led to the investigation.}
...
## Root Cause
{WHY it happens — the 5-whys analysis if applicable}

## Suggested Fix
{Concrete code change or approach to resolve}

## Related
- Session: [[Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-summary]]
- Iteration: [[Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-iter-{N}]]
```

Write to: `Projects/{ProjectName}/Decisions/{YYYY-MM-DD}-bug-{slug}.md`

```
obsidian_update_note({
  targetType: "filePath",
  targetIdentifier: "Projects/{ProjectName}/Decisions/{YYYY-MM-DD}-bug-{slug}.md",
  content: "{rendered bug decision note}",
  modificationType: "wholeFile",
  wholeFileMode: "overwrite",
  createIfNeeded: true
})
...
3. Obsidian persona warnings not yet investigated
4. Uninvestigated files in the error surface
5. Deeper investigation of confirmed bugs (find root cause, not just symptom)
6. Pattern-based search (if found NULL check bug, look for similar patterns elsewhere)
...
When the debug loop terminates (user interrupt, bounded limit reached, or codebase clean):

1. **Write debug session summary to Obsidian** using the session summary template from `obsidian-integration.md`. Include debug-specific fields:

```markdown
---
tags: [autonexus/session, autonexus/debug, autonexus/{project}]
date: {YYYY-MM-DD}
session: {session-id}
goal: debug — {symptom or "autonomous bug hunt"}
...
# Debug Session: {goal summary}

**Iterations:** {total} | **Bugs found:** {count} | **Hypotheses tested:** {count}
**Confirmed:** {count} | **Disproven:** {count} | **Inconclusive:** {count}

## Confirmed Bugs
{List each confirmed bug with severity, location, and one-sentence description}
{Link to Decision note: [[Decisions/{YYYY-MM-DD}-bug-{slug}]]}

## Key Eliminations
{2-3 most informative disproven hypotheses — what they ruled out}

## Observations
{1-2 sentences about patterns: what areas are buggy, what types of bugs dominate}

## Next Steps
- {Suggested follow-up actions}
- {If bugs found: "Run /autonexus:fix --from-debug to fix confirmed bugs"}
```
...
| Flag | Purpose |
|------|---------|
| `--fix` | After finding bugs, switch to /autonexus:fix mode to fix them |
...
debug_score = bugs_found * 15
            ...
            + (obsidian_history_used * 5)
```

Higher = more thorough. Incentivizes breadth (cover more files), depth (test more hypotheses), and knowledge reuse (leverage Obsidian history).
...
| **Ignore Obsidian history** | Past sessions may have already investigated this. Check before reinventing. |
...
Plus Obsidian notes:
- `Projects/{ProjectName}/Decisions/{YYYY-MM-DD}-bug-{slug}.md` — one per confirmed bug
...
When `--fix` is specified, after the debug loop completes, automatically switches to `/autonexus:fix --from-debug` targeting the discovered issues. The `--from-debug` flag tells fix to read findings from the latest debug session.
