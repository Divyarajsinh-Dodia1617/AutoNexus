# Fix Workflow — /autonexus:fix

Autonomous fix loop that takes a broken state and iteratively repairs it until everything passes. One fix per iteration. Atomic, committed, verified, auto-reverted on failure. Each iteration is logged to Obsidian for cross-session memory.

**Core idea:** Detect -> Prioritize -> Fix ONE thing -> Verify -> Keep/Revert -> Repeat until zero errors.

## Trigger

- User invokes `/autonexus:fix`
- User says "fix all errors", "make tests pass", "fix the build", "clean up all warnings"
- User has output from `/autonexus:debug` and wants to fix the findings
...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autonexus:fix` is invoked without explicit `--target`, `--guard`, or `--scope`, you MUST first auto-detect all failures, then use `AskUserQuestion` to gather user input BEFORE proceeding to ANY phase. DO NOT skip this step. DO NOT jump to Phase 1 without completing interactive setup.
...
```
/autonexus:fix
  ├── Phase 0: Session Init (Obsidian connectivity, queue drain, session ID)
  ├── Phase 1: Detect (what's broken? + query Obsidian for recurring issues)
  ...
  └── Phase 8: Log & Repeat (local + Obsidian)
```

## Phase 0: Session Initialization

Run the standard Phase 0 from `obsidian-integration.md`:

1. Obsidian connectivity check
2. Drain sync queue from previous sessions
3. Archive old iteration notes (>30 days)
4. Generate session ID: `{YYYYMMDD}-{HHMM}-{4 random alphanumeric}`
5. Create session folder in Obsidian: `Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/.session-{session_id}`

All steps use the retry-then-queue protocol. See `obsidian-integration.md` for full details.
...
**Obsidian: Query for Recurring Issues**

Before prioritizing, search Obsidian for past fix sessions that addressed the same errors — especially those that were fixed but later reverted (indicating a recurring problem):

```
FOR each detected error (top 10 by severity):
  obsidian_global_search({
    query: "{error description or file:line}",
...
Parse results:
  - Look for notes where the same error was fixed (status: keep) in a prior session
  - Check if the fix was later reverted or the error reappeared
  - If recurring: flag as "RECURRING — previously fixed in session {X}, iteration {N}"
  - Recurring issues get priority boost (+1 level) — they need a deeper fix, not a surface patch
  - Extract the prior fix approach to avoid repeating the same temporary fix
```

Uses retry-then-queue protocol. If fails, proceed with local detection only.

**Output:** `Phase 1: Detected — [N] test failures, [M] type errors, [K] lint errors, [W] warnings, [R] recurring`
...
| 3 | **Recurring issues** | From Obsidian history — need deeper fix |
| 4 | **Type errors** | Type safety prevents cascading bugs |
| 5 | **Test failures** | Tests verify correctness |
| 6 | **Medium/Low bugs** | From debug findings |
| 7 | **Lint errors** | Code quality |
| 8 | **Warnings** | Polish — type "warning" in detection |
...
**Obsidian Pre-Fix Check:**

Before modifying files, run the standard per-iteration Obsidian reads from `obsidian-integration.md`:

1. **Query failed approaches** — search `Projects/{ProjectName}/Iterations/` for past discard/crash notes on the target file. Avoid repeating failed strategies.
2. **Query predict warnings** — search `Projects/{ProjectName}/Predictions/` for persona warnings on the target file. Factor severity into fix approach.
...
| Recurring issue | Address the root cause, not the surface symptom — check prior fix for what went wrong |
...
- Never use any|any escape hatch — use proper narrowed types or generics
- Never delete test coverage — fix the implementation to satisfy tests
...
Recurring issues addressed: 2
```

### Obsidian Logging

**Per-iteration note:** Write a concise iteration note to Obsidian using the iteration note template from `obsidian-integration.md`. Path: `Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-iter-{N}.md`

```
obsidian_update_note({
  targetType: "filePath",
  targetIdentifier: "Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-iter-{N}.md",
  content: "{rendered iteration note — 5-8 lines per template}",
...
Keep iteration notes concise (5-8 lines per the template in `obsidian-integration.md`):

```markdown
---
tags: [autonexus/iteration, autonexus/fix, autonexus/{project}]
date: {YYYY-MM-DD}
iteration: {N}
status: {keep|discard|crash|rework|blocked}
metric: {error_count}
delta: {change}
...
**Change:** {one-sentence: what was fixed and where}
**Result:** {status} — errors {old_count} -> {new_count} ({delta})
**Files:** {comma-separated list of modified files}
**Rationale:** {why this fix was chosen — what error/hypothesis motivated it}
**Next:** {what remains or what to try next}
```

### Zero-Error "Clean State" Decision Note

When the error count hits zero, write a Decision note to Obsidian documenting the clean state achievement:

```markdown
---
tags: [autonexus/decision, autonexus/fix-complete, autonexus/{project}]
date: {YYYY-MM-DD}
status: accepted
session: {session-id}
baseline_errors: {N}
...
# Decision: Clean State Achieved — {goal summary}

## Context
Fix session started with {N} errors ({breakdown by category}). All errors resolved in {iterations} iterations.

## What Was Fixed
{Categorized list of fixes applied:}
- **Build:** {count} fixes — {brief descriptions}
- **Type:** {count} fixes — {brief descriptions}
- **Test:** {count} fixes — {brief descriptions}
- **Lint:** {count} fixes — {brief descriptions}
- **Bugs:** {count} fixes — {brief descriptions}

## Blocked / Skipped
{List any items that were skipped, or "None — all items resolved."}

## Guard Status
Guard command `{command}` passed on all iterations. No regressions introduced.

## Consequences
- Codebase is at zero known errors
- All guards passing
- {Any trade-offs or notes about the fixes}
```

Write to: `Projects/{ProjectName}/Decisions/{YYYY-MM-DD}-fix-clean-{slug}.md`

All Obsidian writes use the retry-then-queue protocol. If they fail, the local TSV still has all data — no information is lost.

**Completion detection:**
```
IF current_errors == 0:
  PRINT "=== All Clear — Zero Errors ==="
  Write "Clean State" Decision note to Obsidian
  STOP (even in unbounded mode)
```

## Session End — Obsidian Summary

When the fix loop terminates (zero errors, user interrupt, or bounded limit reached):

1. **Write fix session summary to Obsidian** using the session summary template from `obsidian-integration.md`. Include fix-specific fields:

```markdown
---
tags: [autonexus/session, autonexus/fix, autonexus/{project}]
date: {YYYY-MM-DD}
session: {session-id}
goal: fix — {target description}
...
reworks: {count}
blocked: {count}
recurring_fixed: {count}
---

# Fix Session: {goal summary}

**Baseline:** {N} errors -> **Final:** {M} errors ({delta}, {percentage}%)
**Iterations:** {total} (Keeps: {n} | Discards: {n} | Reworks: {n} | Blocked: {n})

## Top Wins
{List top 3-5 kept iterations by delta, with one-sentence descriptions}

## Recurring Issues Addressed
{List recurring issues that were given deeper fixes this session}

## Key Failed Approaches
{List 2-3 most informative discards — what didn't work and why}

## Blocked Items
{Items that failed 3+ attempts — with suggestion to run /autonexus:debug}

## Observations
{1-2 sentences about patterns: what types of fixes worked, what areas are fragile}

## Next Session Suggestions
{2-3 concrete suggestions for follow-up}
```

Write to: `Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-summary.md`

2. **Append daily note mini-summary** per `obsidian-integration.md`.
3. **Final queue drain** — attempt full drain of `obsidian-sync-queue.md`.
...
DETECTING:
  ...
  -> Query Obsidian for recurring issues
  -> If zero errors found -> DONE (nothing to fix)
  -> If errors found -> PRIORITIZING

PRIORITIZING:
  -> Sort errors by priority table (recurring issues boosted)
  -> Group cascading errors (fixing one fixes others)
  -> Pick first unfixed item -> FIXING

FIXING:
  -> Query Obsidian for failed approaches + predict warnings on target file
...
DECIDING:
  -> delta > 0 AND guard passes -> KEEP -> log "fixed" -> write Obsidian iter note -> LOOP
  ...
  -> current_errors == 0 -> write "Clean State" Decision note -> DONE

DONE:
  -> Generate summary.md
  -> Write session summary to Obsidian
  -> Print fix_score
  -> Suggest /autonexus:debug for blocked items
```
...
| Repeat the same fix that was reverted | If it didn't work before, it won't work now | Check Obsidian history, try a different approach |
...
bonus_score = 0
  ...
  bonus_score += (recurring_fixed ? 5 : 0)            # addressed recurring issues
```
...
Attempt 3: FAIL -> Escalate:
  ...
  5. SUGGEST: /autonexus:debug on the specific error for root cause analysis
```
...
/autonexus:fix --category test --guard "tsc --noEmit"
...
Plus Obsidian notes:
- `Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-iter-{N}.md` — one per iteration
- `Projects/{ProjectName}/Iterations/{YYYY-MM-DD}/{session-id}-summary.md` — session summary
- `Projects/{ProjectName}/Decisions/{YYYY-MM-DD}-fix-clean-{slug}.md` — if zero errors achieved

## Extended Chaining Patterns

```bash
# Full pipeline: predict -> debug -> fix -> ship
...
/autonexus:fix --category warning --skip-lint

# CI-specific fix: re-run on CI failure
/autonexus:fix --target "npm run ci" --guard "npm test"

# After dependency upgrade: fix cascade
npm upgrade && /autonexus:fix --category type --category test
```
...
- auth/token-refresh.ts — circular dependency blocks type resolution
  -> Suggested: /autonexus:debug --scope auth/token-refresh.ts
